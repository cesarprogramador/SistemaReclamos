...
CREATE TABLE `historialreclamos` (
  ...
  `repuesta` varchar(2) DEFAULT NULL,
...
insert  into `problematicareclamos`(`idproblematicareclamo`,`problematicareclamo`,`idtiporeclamo`,`SI`,`NO`,`orden`) values 
...
CREATE TABLE `reclamos` (
  ...
  `idtiporeclamo` int(11) DEFAULT NULL,
  `numeroreferencia` varchar(10) DEFAULT NULL,
  `accion` varchar(1) DEFAULT NULL,
  `fechaaccion` datetime DEFAULT NULL,
...
(1,'2017-11-06',NULL,2,1,2,'R0001','N','2017-11-06 16:59:06');
...
BEGIN
	IF (_idproblematica<=0) THEN
		PREPARE STMT FROM " SELECT idproblematicareclamo,problematicareclamo,SI,NO 
				    FROM problematicareclamos 
			            WHERE (orden=(SELECT MIN(orden) 
						  FROM problematicareclamos 
						  WHERE (idtiporeclamo=?)))
				    LIMIT ?,?";
	...
	ELSE
		PREPARE STMT FROM " SELECT idproblematicareclamo,problematicareclamo,SI,NO 
				    FROM problematicareclamos 
			            WHERE (idtiporeclamo=?) AND (idproblematicareclamo=?)
				    LIMIT ?,?";
		SET @idtiporeclamo = _idtiporeclamo; 
		SET @idproblematica = _idproblematica; 
		SET @pag = _pag; 
		SET @cantfil = _cantfil; 
		EXECUTE STMT USING @idtiporeclamo,@idproblematica,@pag,@cantfil;
		DEALLOCATE PREPARE STMT;
	END IF;
END */$$
DELIMITER ;
